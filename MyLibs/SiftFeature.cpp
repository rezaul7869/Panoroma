...
// ToDo 1: Fill out this function
	...
	int idx = -1;
	int i = 0;
	vector< const CFeature* >::const_iterator CFIterator;
	vector< const CFeature* >::const_iterator CFEnd = set.end();
	for(CFIterator = set.begin(); CFIterator != CFEnd; CFIterator++, i++)
	{
		double sqrDiff = 0.0;
		const CFeature* cf = *CFIterator;
		for(int j = 0; j < ft->m_numDim && j < cf->m_numDim; j++)
		{
			double diff =  ft->d[j] - cf->d[j];
			sqrDiff += diff * diff;
		}
		if(sqrDiff < minDiff)
		{
