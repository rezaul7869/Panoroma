...
void PoseFromHomography( double* R, const CHomography& homo, double* K1, double* K2 )
{
    ...
    CvMat k1Mat = cvMat(3,3,CV_64F,K1);
    CvMat k2Mat = cvMat(3,3,CV_64F,K2);
    double invresultInit[3][3];
    double temp2Init[3][3];
    double resultInit[3][3];

    CvMat temp1 = cvMat(3, 3, CV_64F,invresultInit);
    CvMat temp2 = cvMat(3, 3, CV_64F, temp2Init);
    CvMat result = cvMat(3, 3, CV_64F, resultInit);
    cvInvert(&k2Mat, &temp1);
    cvMatMul((CvMat*)&homo, &k1Mat, &temp2);
    cvMatMul(&temp1, &temp2, &result);

    memcpy(R, result.data.db, sizeof(double) * 9);
}
				  ...
				  int numImage )
{
    ...
    {
        MatchArray aryInlier = aryMatch[i];
        int aryMatchSize = aryInlier.size();
        for( int j=0; j < aryMatchSize; j++ )
	    {
            const CFeature* ft1 = set[i-1][ aryInlier[j].first];
		    const CFeature* ft2 = set[i][ aryInlier[j].second ];

		    double dir[3];
		    cam[i-1].GetRayDirectionWS( dir, cvPoint2D64f( ft1->x, ft1->y ) );
		
		    // the 3d position
		    CvPoint3D64f pt3_1 = cvPoint3D64f( dir[0]*radius, dir[1]*radius, dir[2]*radius );

		    cam[i].GetRayDirectionWS( dir, cvPoint2D64f( ft2->x, ft2->y ) );
            CvPoint3D64f pt3_2 = cvPoint3D64f( dir[0]*radius, dir[1]*radius, dir[2]*radius );
            /*for(int k = 0; k < numImage; k++)
            {
                CvPoint2D64f pt;
                pt = cam[k].GetProjection(pt3);
                ba.SetPointProjection( pt3, k, cvPoint2D64f( pt.x,pt.y ) );
            }*/
            ba.SetPointProjection( pt3_1, i-1, cvPoint2D64f( ft1->x, ft1->y ) );
		    ba.SetPointProjection( pt3_1, i, cvPoint2D64f( ft2->x, ft2->y ) );
		    ba.SetPointProjection( pt3_2, i-1, cvPoint2D64f( ft1->x, ft1->y ) );
		    ba.SetPointProjection( pt3_2, i, cvPoint2D64f( ft2->x, ft2->y ) );
        }
    }
